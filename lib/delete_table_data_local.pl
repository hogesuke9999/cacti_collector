...
sub delete_table_data_local {
	...
	print "***** Delete Table Name = data_local *****\n";

	$sql_w = "select ref_id from data_local where ref_hostname = '" . $db_r_host . "';";
	...
	while (my $arr_ref = $sth_w->fetchrow_arrayref) {
		...
		$sql_r = "select count(*) from data_local where id = '" . $TABLE_ref_id . "';";
		...
		if($id_exist < 1) {
			$sql_w2 = "delete from data_local where ref_id = '" . $TABLE_ref_id . "' and ref_hostname = '" . $db_r_host . "';";
			print "Delete : " . $sql_w . "\n";
	        	$db_w->do($sql_w2);
		}
	}
